...
let PlayNow = React.createClass({
  ...
  componentWillMount(){
    ...
    this.socket.on('winner', function(data) {
      ...
      if (_this.state.current_user.email == data.winner.email){
        msg += "Congrats! You have won. With score: " + data.winner.score;
      }else{
        msg += "Winner " + data.winner.email + " with score: " + data.winner.score;
      }
      winnerScore = data.winner.score;
      winnerEmail = data.winner.email;
      alert(msg);
    });
  ...
  renderGame(){
    return (
        <div>
            <a href="/profile"><span className="fa fa-arrow-left fa-4x"></span>Back to home</a>
           <div className="row">
            {this.renderUsers()}
          </div>
